document.addEventListener('DOMContentLoaded', () => {
    ...
    const createPanel = async () => {
        ...
        view.className = 'w-full h-full border-0';
        view.src = 'about:blank';
        
        if (isElectron) {
            view.allowpopups = true;
        } else {
            // For iframes, you might want to set sandbox attributes
            // view.sandbox = 'allow-scripts allow-same-origin allow-popups allow-forms';
        }
        ...
        const newPanel = { id: panelId, element: wrapper, view: view };
        ...
        if (isElectron) {
            ...
            view.addEventListener('dom-ready', async () => {
                ...
                const fullscreenScript = await window.electronAPI.getFullscreenScript();
                view.executeJavaScript(fullscreenScript);
            });

            // Handle fullscreen events from webview
            view.addEventListener('enter-html-full-screen', (e) => {
